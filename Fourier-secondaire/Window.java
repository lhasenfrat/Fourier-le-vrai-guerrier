...
public class Window extends JFrame implements ActionListener {
    ...
    public Window(){
        ...
        /*Panel affichage*/
        panelshow = new DispPan(new LinkedList<Complexe>(), new LinkedList<Complexe>());
    ...
    public void actionPerformed (ActionEvent e){
        ...
        if ((e.getSource() == buttonstart)) {
			String texteCadre = "";
			panelshow.origine = paneldraw.listepoints;
    ...
    public static LinkedList<Complexe> tFourier(LinkedList<Complexe> l, int nbVect){	//méthode qui effectue la transformée de fourier
        ...
        for(int i = 0 ; i < nbVect ; i++){
            ...
            double t = 0;
            for(Complexe d : l){
				for(int k = 0 ; k < multiNbPoints ; k++){
					Complexe c = new Complexe(pass.getRe() + (d.getRe() - pass.getRe())*k/multiNbPoints, pass.getIm() + (d.getIm() - pass.getIm())*k/multiNbPoints);
					Complexe sc = new Complexe(1, c.getRho(), c.getTheta() - (i*2*Math.PI*t/(multiNbPoints*nbPoints)));
					integrale.sommeV1(sc);
					t++;
				}
				pass = d;
            }
            integrale.setRho(integrale.getRho()/(multiNbPoints*nbPoints));
